...
class Baza {
    ...
    function multiQuery($upit) {
        if (!$this->veza->multi_query($upit)) {
            echo "Multi query failed: (" . $this->veza->errno . ") " . $this->veza->error;
        }

        do {
            if ($res = $this->veza->store_result()) {
                return $res->fetch_assoc();
            }
        } while ($this->veza->more_results() && $this->veza->next_result());
    }
}
?>